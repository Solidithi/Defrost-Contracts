...
async function main(config: CreateLaunchpoolConfig) {
    ...
    try {
        const xcmOracle = await contract.xcmOracleAddress();
        console.log("XCM Oracle address:", xcmOracle);
        
        // Try to verify the XCM oracle works correctly by calling directly
        const mockXCMOracle = await ethers.getContractAt("IXCMOracle", xcmOracle);
        
        try {
            const currencyId = await mockXCMOracle.getCurrencyIdByAssetAddress(config.nativeAssetAddress);
    ...
    console.log("Is signer project owner?", project.projectOwner.toLowerCase() === signer.address.toLowerCase());
    ...
    const currentAllowance = await projectToken.allowance(signer.address, config.proxyAddress);
    ...
    try {
        const estimatedGas = await contract.createLaunchpool.estimateGas(params);
        ...
        try {
            const poolId = (await contract.nextPoolId()) - 1n;
        ...
        if ((error as any).message) {
            ...
            if ((error as any).message.includes("reverted")) {
                ...
                } else if ((error as any).message.includes("reverted with custom error")) {
                    console.error("Reverted with custom error");
                } else if ((error as any).message.includes("reverted with panic code")) {
                    const panicMatch = (error as any).message.match(/reverted with panic code 0x([0-9a-f]+)/);
                    if (panicMatch) {
                        ...
                        console.error(`Panic reason: ${panicReasons[panicCode] || "Unknown panic code"}`);
                    }
                }
            }
        }
        ...
        if ((error as any).transaction) {
            console.log("Trying to simulate the failed transaction to get more details...");
            try {
...
main({
    proxyAddress: "0xD1b11fE68a7e363E5564aB86AB496Ec9C4eB125d",
